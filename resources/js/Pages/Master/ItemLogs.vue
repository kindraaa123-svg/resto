...
import { Search, Package, Building, Layers, ArrowUpCircle, ArrowDownCircle, Filter, X } from '@lucide/vue'
...
const applyFilters = () => {
    router.get(route('item.item-logs.index'), {
        filter: {
...
<template>
    <Head :title="ui.item_logs || 'Item Logs'" />

    <div class="max-w-7xl mx-auto space-y-8">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-black tracking-tight text-primary uppercase">{{ ui.item_logs || 'Item Logs' }}</h1>
                <p class="text-muted-foreground text-sm font-medium uppercase tracking-widest mt-1">{{ ui.track_item_movements || 'Track Item Movements (In/Out)' }}</p>
            </div>
        </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black uppercase text-muted-foreground ml-1">{{ ui.search_item || 'Search Item' }}</label>
                        <div class="relative">
        ...
        <Card class="border-none shadow-sm bg-card/50 overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-secondary/20 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">
                            ...
                            <th class="px-6 py-4">{{ ui.items || 'Item' }}</th>
                            <th class="px-6 py-4">{{ ui.branch || 'Branch' }}</th>
                            <th class="px-6 py-4">{{ ui.type || 'Type' }}</th>
                            <th class="px-6 py-4 text-center">{{ ui.quantity || 'Quantity' }}</th>
                            <th class="px-6 py-4 text-right">{{ ui.price || 'Price' }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-secondary/30">
                        <tr v-for="item in itemLogs?.data || []" :key="item.itemlogid" class="group hover:bg-white/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-sm font-black text-primary">{{ Math.round(item.qty) }}</span>
                            </td>
                            <td class="px-6 py-4 text-right text-xs font-black text-primary">
                                <span v-if="item.type === 'IN'">{{ formatCurrency(item.price) }}</span>
                                <span v-else class="text-[10px] uppercase tracking-widest text-muted-foreground/50">—</span>
                            </td>
                        </tr>
                        <tr v-if="!itemLogs?.data || itemLogs.data.length === 0">
                            <td colspan="7" class="px-6 py-12 text-center text-muted-foreground italic text-sm">
                                {{ ui.no_item_logs || 'No item logs found.' }}
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="p-6 border-t border-secondary/50">
                <Pagination :links="itemLogs?.links || []" />
            </div>
        </Card>
    </div>
</template>
